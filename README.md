...
with curl: 
    ...
    curl -O -L https://github.com/stokpop/lograter/releases/download/1.5.5/lograter-exec-1.5.5.jar

use java 11+ to run lograter:

    java -jar lograter-exec-1.5.5.jar
...
process this file:
     ...
     java -jar lograter-exec-1.5.5.jar access -gt -single-mapper NASA_access_log_Jul95.gz  
...
The order of the options on the command line is important. There are general options and command specific options.
    ...
    java -jar lograter-exec-1.5.5 [general options] [command] [command options] [input file(s)]
...
Now call LogRater as:
    ...
    java -jar lograter-exec-1.5.5.jar @latency.options cloud-foundry.log  
