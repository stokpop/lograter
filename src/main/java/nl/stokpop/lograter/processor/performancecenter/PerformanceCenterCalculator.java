...
public final class PerformanceCenterCalculator {
    ...
    public static Integer determineAggregationPeriod(File resultsDatabaseFile) {
        try {
            return ResultConfigReader
                    .read(resultsDatabaseFile.getAbsoluteFile().getParentFile().toPath())
                    .getAggSecGran();
        } catch (IOException e) {
    ...
    public static long calculateStartTimeSecEpoch(Path unzippedResultsDir) {
        try {
            ResultConfig resultConfig = ResultConfigReader.read(unzippedResultsDir);
